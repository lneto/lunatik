...
local function rst(skb)
	...
	if proto == TCP then
		...
		if dport == PORT then
			...
			-- truncate payload: RST carries no data
			local tcp_hdr = ((pkt:getuint8(ihl + 12) >> 4) & 0x0F) * 4
			local new_len = ihl + tcp_hdr
			skb:resize(new_len)
			pkt:setuint16(2, byteorder.hton16(new_len))
			skb:checksum()
			skb:forward()
